...
export class EngineSupervisor {
  ...
  private async startInternal():Promise<EngineSnapshot>{
    ...
    try{
      ...
      if(identity.service!=="forge-desktop-bridge")throw new Error("unexpected service identity");
  ...
  private async fetchBounded(origin:string,path:string,tokenValue:string|undefined,timeoutMs:number):Promise<Response>{return await fetch(`${origin}${path}`,{redirect:"manual",signal:AbortSignal.timeout(timeoutMs),headers:tokenValue?{"x-forge-bridge-authorization":`Bearer ${tokenValue}`,Origin:"http://forge-desktop.internal"}:{}})}
