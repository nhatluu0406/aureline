...
Chưa rewrite HTML/JS/CSS body, absolute WebSocket URL hoặc CSP. Buffer/rewrite body sẽ phá streaming/compression và dễ làm hỏng extension. Production nên cấu hình request/forwarded host/root path để Gradio sinh relative/proxy URLs, rồi real-Forge smoke xác minh. Nếu Gradio 4.40/extension hardcode backend origin, cần route-specific rewrite có test; không bật generic string replacement.
...
- Chưa chạy Forge/Gradio 4.40 thực hoặc packaged Electron; Uvicorn Config wrapper và `webRequest` injection còn cần integration smoke.
- Không chứng minh absolute URL/CSP/root path/UI reload của Forge qua proxy.
- Không chạy representative extension, upload file thực của Gradio hoặc range download.
...
Trước merge production cần bounded real-Forge compatibility smoke chứng minh wrapper thực sự nằm ngoài app từ first request, Gradio SSE/upload/file/range/UI reload hoạt động, exact proxy origin không xuất hiện sai trong config, representative extension route được bảo vệ, và packaged Electron `webRequest` inject cả HTTP lẫn WebSocket mà page JS không đọc token.

## 23. Proposed next task

Task kế tiếp duy nhất: **bounded real-Forge compatibility smoke** với runtime đã materialize, không model/inference. Mục tiêu là chạy Forge `--ui-debug-mode --api` qua launcher adapter + proxy trên temporary data root và xác minh Gradio page/assets/SSE/upload/file/redirect/UI reload cùng middleware pre-bind. Nếu runtime vẫn chưa sẵn sàng mà phải download dependency lớn, task phải dừng ở precondition thay vì bootstrap ngầm.
