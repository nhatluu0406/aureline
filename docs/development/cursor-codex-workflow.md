...
- task có thể cho phép direct-main, commit và push rõ ràng; khi đó không tạo branch/worktree phụ và vẫn phải fetch, kiểm tra divergence, giữ unrelated changes, rồi push không force.
...
Cursor Project Rules chính thức nằm trong `.cursor/rules/`, là hướng dẫn ngắn theo glob và bổ sung cho `AGENTS.md`; không dùng legacy `.cursorrules`.
